...
void get_spectrum(int8_t *pol1_data, int8_t *spectrum){
    ...
    spectrum[0] = Nby2;
    spectrum[1] = Nby2_imag;

    for(int i = 2; i < 2*NCHAN; i++){
        spectrum[i] = pol1_data[2*NCHAN - i + 1];
    }
...
int main(int argc, char* argv[]){
    ...
    //OBS_BW
    char command[128];
    snprintf(command, sizeof(command), "sed -i 's/^OBSBW=.*/OBSBW=%.1f/' main_header.txt", (double) (-1*bandwidth));
    ...
    //CHAN_BW
    snprintf(command, sizeof(command), "sed -i 's/^CHAN_BW=.*/CHAN_BW=%.11Lf/' main_header.txt", -1*((long double)bandwidth/(long double)NCHAN));
